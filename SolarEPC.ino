...
// includes moving averages thanks to Chris - RX14
#include <JeeLib.h>
	...
	if (rf12_recvDone()) {
		if (rf12_crc == 0 && (rf12_hdr & RF12_HDR_CTL) == 0)
		{
			...
			if (node_id == emonTx_NodeID)  { //check data is coming from node with the correct ID
				...
				int power = emontx.power4 + emontx.power2;
				
				average = ((SAMPLES - 1) * average + power)
				
				Serial.print("SolarOutput - Watts: "); Serial.println(power);
				Serial.print("Average SolarOutput - Watts: "); Serial.println(average);
				Serial.print("GridVoltage: "); Serial.println(emontx.Vrms / 100);
				Serial.println(" ");
				if (average < 1600) { // If Solar is less than 1600w set the pot to 0 ohms switching off charging
				...
				else if (average > 1600 < 3300 ) { // If Solar is 1600 - 3000W set the pot to 196 ohms 6A
				...
				else if (average > 3300) { // If Solar is  more than 3300W set the pot to 729 ohms 16A
					digitalPotWrite(3, 8); // 16A
